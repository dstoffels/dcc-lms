...
import { LabTaskAttempt as LabTaskAttemptType } from '../../lib/types';
...
const LabAttempt = ({ attempt }: LabAttemptContentProps) => {
	...
	const handleComplete = async () => {
		try {
			await api.post(`/labs/1/attempts/${attempt.id}/complete`, { code });
			router.refresh();
	...
	const handleAssistant = async () => {
		try {
			await axios.get(`/labs/1/attempts/${attempt.id}/assistant`);
			router.refresh();
		} catch (error: AxiosError | any) {
			console.log(error.response.data);
		}
